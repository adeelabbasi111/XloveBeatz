...
def admin_discount_add():
    ...
    # Send email to all users (including admins so they can test it)
...
def migration_run():
    ...
    def generate():
        yield f"data: {json.dumps({'msg': 'Starting bulk optimization...', 'progress': 0})}\n\n"
        
        from helpers.models import Genre, Product, BeatDetail, db
        import shutil
        from PIL import Image
            ...
            if g.image_path:
                ...
                if os.path.exists(abs_path):
                    ...
                    try:
                        ...
                        new_filename = optimize_image_size_duel(img, dest_dir, base_name, 800, 800, 85)
                        if new_filename:
                            new_rel_path = f"data/genres/{new_filename}" if 'genres' in g.image_path else g.image_path.replace(os.path.basename(g.image_path), new_filename)
                            if new_rel_path != g.image_path:
                                old_path = g.image_path
                                g.image_path = new_rel_path
                                db.session.commit()
                                old_abs = os.path.join(current_app.root_path, 'static', old_path)
                                if os.path.exists(old_abs) and old_abs != os.path.join(current_app.root_path, 'static', new_rel_path):
                                    os.remove(old_abs)
                    except Exception as e:
                        yield f"data: {json.dumps({'msg': f'Error on Genre {g.name}: {str(e)}'})}\n\n"
        ...
        for i, p in enumerate(products):
            if p.cover_image:
                abs_path = os.path.join(current_app.root_path, 'static', p.cover_image)
                if os.path.exists(abs_path):
                    yield f"data: {json.dumps({'msg': f'Optimizing Product: {p.name}', 'progress': int(20 + (i/max(total_products,1))*30)})}\n\n"
                    try:
                        img = Image.open(abs_path)
                        dest_dir = os.path.dirname(abs_path)
                        base_name = os.path.splitext(os.path.basename(abs_path))[0]
                        new_filename = optimize_image_size_duel(img, dest_dir, base_name, 1000, 1000, 85)
                        if new_filename:
                            new_rel_path = p.cover_image.replace(os.path.basename(p.cover_image), new_filename)
                            if new_rel_path != p.cover_image:
                                old_path = p.cover_image
                                p.cover_image = new_rel_path
                                db.session.commit()
                                old_abs = os.path.join(current_app.root_path, 'static', old_path)
                                if os.path.exists(old_abs) and old_abs != os.path.join(current_app.root_path, 'static', new_rel_path):
                                    os.remove(old_abs)
        ...
        # 3. Beats Images
        beats = BeatDetail.query.all()
        total_beats = len(beats)
        for i, b in enumerate(beats):
            if b.beat_image:
                abs_path = os.path.join(current_app.root_path, 'static', b.beat_image)
                if os.path.exists(abs_path):
                    yield f"data: {json.dumps({'msg': f'Optimizing Beat Image: {b.product.name if b.product else b.id}', 'progress': int(50 + (i/max(total_beats,1))*20)})}\n\n"
                    try:
                        img = Image.open(abs_path)
                        dest_dir = os.path.dirname(abs_path)
                        base_name = os.path.splitext(os.path.basename(abs_path))[0]
                        new_filename = optimize_image_size_duel(img, dest_dir, base_name, 300, 300, 60)
                        if new_filename:
                            new_rel_path = b.beat_image.replace(os.path.basename(b.beat_image), new_filename)
                            if new_rel_path != b.beat_image:
                                old_path = b.beat_image
                                b.beat_image = new_rel_path
                                db.session.commit()
                                old_abs = os.path.join(current_app.root_path, 'static', old_path)
                                if os.path.exists(old_abs) and old_abs != os.path.join(current_app.root_path, 'static', new_rel_path):
                                    os.remove(old_abs)
                ...
                if os.path.exists(abs_path):
                    ...
                    try:
                        ...
                        if result.returncode == 0 and os.path.exists(temp_out):
                            os.replace(temp_out, abs_path)
                    except Exception as e:
                        pass
                        
        yield f"data: {json.dumps({'msg': 'All optimizations complete!', 'progress': 100})}\n\n"
